...
* Maintain user details who purchase or sell the stocks
* Print a Stock Report with total value of each Stock and the total value of Stock.
* Maintain the transaction using stack.
**/
...
func main() {
	...
	for flag {
		...
		fmt.Println("7.To view Stack details")
		fmt.Println("8. Exit ")
...
// @Push - This function is used to add the transaction details to the stack.

func (l *LinkedList) Push(s1 *stock1) {

	if l.count == 0 {
		l.head = s1 // If there are no elements in stock.
	} else {
		// currentPost := l.head
...
//@Pop-To delete the last transaction of the file.
...
func (l *LinkedList) AddToStack(name string, Number_of_shares int, Share_price int, Status string) {
	//fmt.Println("Add to stack")
	s := stock1{
		Name:             name,
...
func SaveToStock(s1 []stock) {
	...
	err = ioutil.WriteFile("stock.json", result, 0)
}
...
func (l *LinkedList) Buy() {
	...
	if check == 0 {
		...
		if total_price > u1[0].Balance {
			...
			fmt.Println()
			l.AddToStack(company_name, Number_of_shares, total_price, "Purchased")
		}
		SaveToStock(s1)
		SaveToUser(u1)
	}
...
func (l *LinkedList) Sell() {
	...
	if u1[0].Shares > 0 && u1[0].Shares < Number_of_shares {
		...
		if check == 0 {
			...
			u1[0].Balance = u1[0].Balance + total_price
			l.AddToStack(company_name, Number_of_shares, total_price, "Sold")
